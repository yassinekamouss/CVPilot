...
/**
 * Maps the `error` search-param values emitted by NextAuth (or our own
 * signIn callback redirects) to user-friendly, non-technical messages.
 *
 * `existingProvider` is the slug returned from our signIn callback
 * (e.g. "google", "linkedin") — never an email address.
 */
function resolveErrorMessage(
  ...
  if (error === "AccountConflict") {
    const providerLabel =
      (existingProvider && AUTH_PROVIDER_LABELS[existingProvider as keyof typeof AUTH_PROVIDER_LABELS]) ??
      existingProvider ??
      "another provider";

    return `You previously signed in with ${providerLabel}. Please sign in with ${providerLabel} to access your account, then connect additional providers from your account settings.`;
  }
  ...
  return (
    staticMessages[error] ??
    "An unexpected error occurred. Please try again."
  );
}

export default async function LoginPage({ searchParams }: LoginPageProps) {
  const { error, existingProvider } = await searchParams;
  const errorMessage = resolveErrorMessage(error, existingProvider);
          ...
          <div className="space-y-3">
            ...
            <form
              action={async () => {
                "use server";
                try {
                  await signIn(DEFAULT_AUTH_PROVIDER, { redirectTo: APP_ROUTES.dashboard });
                } catch (err) {
            ...
            <form
              action={async () => {
                "use server";
                try {
                  await signIn("linkedin", { redirectTo: APP_ROUTES.dashboard });
                } catch (err) {
                  if (err instanceof AuthError) throw err;
                  throw err;
                }
